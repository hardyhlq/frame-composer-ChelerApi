...
class CDBs {
	...
	protected static $transaction_flag = false; //事务标识
	
	/**
	 *
	 * @var \frame\dao\db\driver\IDriver
	 */
	public $db = NULL;
	...
	protected function getHandle($sql) {
		...
		if ($db_type == 1) { // 主从
			if ($this->is_insert ( $sql ) || self::$transaction_flag) {
	...
	private function get_driver($driver = 'Mysqli') {
		return ChelerApi::loadClass ( '\frame\dao\db\driver\C' . $driver );
	}
